...
void VanAnalyzerResults::GenerateBubbleText( U64 frame_index, Channel& channel, DisplayBase display_base )
{
    ...
    U64 frameData1 = frame.mData1;
    U64 frameData2 = frame.mData2;
    U8 frameType = frame.mType;
    ...
    {
    case StartOfFrame:
        AnalyzerHelpers::GetNumberString( frameData1, display_base, 8, number_str, 128 );
        ss << "SOF: " << number_str;
        AddResultString( ss.str().c_str() );
        break;
    case IdentifierField:
        AnalyzerHelpers::GetNumberString( frameData1, display_base, 12, number_str, 128 );
        ss << "IDEN: " << number_str;
        AddResultString( ss.str().c_str() );
        break;
    case CommandField:
        AnalyzerHelpers::GetNumberString( frameData1, display_base, 4, number_str, 128 );
        // EXT(1), RAK (0-no, 1-yes), RW (0-W,1-R), RTR (0-data, 1-no data)
        Command com;
        com.Value = frameData1;
        if( com.data.RAK == 1 )
        {
            command[ 0 ] = 'A';
        }
    ...
    case DataField:
        AnalyzerHelpers::GetNumberString( frameData1, display_base, 8, number_str, 128 );
        ss << "DATA(" << frameData2 << "): " << number_str;
        AddResultString( ss.str().c_str() );
        break;
    case FCSField:
        AnalyzerHelpers::GetNumberString( frameData1, display_base, 8, number_str, 128 );
        ss << "FCS(" << frameData2 << "): " << number_str;
        AddResultString( ss.str().c_str() );
        break;
    case EndOfData:
        AnalyzerHelpers::GetNumberString( frameData1, display_base, 2, number_str, 128 );
        ss << "EOD: " << number_str;
        AddResultString( ss.str().c_str() );
        break;
    case AckField:
